import { Bot, ChevronDown, Database, GitCommitHorizontal, GitPullRequest, MoveRight } from 'lucide-react';
...
export function PageLineage({
  ...
  const summary = [
    lineage.sources.length ? `${lineage.sources.length} ${lineage.sources.length === 1 ? 'source' : 'sources'}` : '',
    lineage.agents.length ? `${lineage.agents.length} ${lineage.agents.length === 1 ? 'agent' : 'agents'}` : '',
    lineage.commit ? '1 commit' : '',
    lineage.review ? '1 review' : '',
  ].filter(Boolean).join(' · ');

  return (
    <details
      aria-label="Page lineage"
      style={{ marginTop: 36, paddingTop: 12, borderTop: `1px solid ${C.lineSoft}` }}
    >
      <summary style={{
        display: 'flex', alignItems: 'center', gap: 8, width: 'fit-content',
        padding: '5px 2px', color: C.muted, cursor: 'pointer', listStyle: 'none',
        fontSize: 12.5, userSelect: 'none',
      }}>
        <GitCommitHorizontal aria-hidden size={14} color={C.accent} />
        <span style={{ color: C.ink2, fontWeight: 650 }}>Lineage</span>
        <span style={{ color: C.faint }}>{summary}</span>
        <ChevronDown aria-hidden size={13} color={C.faint} />
      </summary>
      <div style={{ padding: '10px 0 4px' }}>
        <div style={{ marginBottom: 9, color: C.faint, fontSize: 11.5 }}>
          Evidence behind the current published page.
        </div>
