...
impl Role {
    ...
    pub fn can_manage(&self) -> bool { *self >= Role::Manager }
    pub fn can_edit(&self) -> bool { *self >= Role::Editor }
    pub fn can_view(&self) -> bool { *self >= Role::Viewer }
    pub fn can_delete_workspace(&self) -> bool { *self == Role::Owner }
    pub fn can_transfer_ownership(&self) -> bool { *self == Role::Owner }

    /// Higher role manages lower role (strictly greater).
    pub fn can_manage_role(&self, target: Role) -> bool { *self > target }
}
...
pub async fn add_member_direct(pool: &PgPool, workspace_id: Uuid, user_id: Uuid, role: &str, invited_by: Uuid) -> sqlx::Result<()> {
    sqlx::query(
...
pub async fn add_member_public_join(pool: &PgPool, workspace_id: Uuid, user_id: Uuid, role: &str, invited_by: Uuid) -> sqlx::Result<()> {
    sqlx::query(
...
pub async fn create_invitation(pool: &PgPool, workspace_id: Uuid, email: &str, role: &str, invited_by: Uuid, invited_user_id: Uuid) -> sqlx::Result<Invitation> {
...
pub async fn resolve_user_identifier(pool: &PgPool, identifier: &str) -> sqlx::Result<Option<Uuid>> {
...
pub async fn find_invitations_by_workspace(pool: &PgPool, workspace_id: Uuid) -> sqlx::Result<Vec<Invitation>> {
    sqlx::query_as::<_, Invitation>(
        "SELECT * FROM invitations WHERE workspace_id = $1 ORDER BY created_at DESC"
    )
    .bind(workspace_id).fetch_all(pool).await
    .map_err(|e| { tracing::error!("DB find_invitations_by_workspace failed: {e}"); e })
}
...
pub async fn expire_stale_invitations(pool: &PgPool) -> sqlx::Result<u64> {
    ...
    .execute(pool).await
    .map_err(|e| { tracing::error!("DB expire_stale_invitations failed: {e}"); e })?;
    Ok(rows.rows_affected())
}

pub async fn touch_last_active(pool: &PgPool, workspace_id: Uuid, user_id: Uuid) -> sqlx::Result<()> {
    sqlx::query(
...
mod tests {
    ...
    fn test_role_can_manage_role_matrix() {
        ...
        for &a in &all {
            for &b in &all {
                if a != b {
                    assert!(a.can_manage_role(b) != b.can_manage_role(a) || !a.can_manage_role(b),
                        "asymmetric: {:?} vs {:?}", a, b);
                }
            }
        }
    ...
    async fn test_pool() -> Option<PgPool> {
        ...
        let _ = sqlx::raw_sql(include_str!("migrations/009_role_management.sql")).execute(&pool).await;
        Some(pool)
    }
    ...
    async fn test_add_member_with_role() {
        ...
        let ws = create(&pool, "Test WS", &slug, "public", user_a).await.unwrap();
        add_member(&pool, ws.id, user_b, "viewer", user_a).await.unwrap();
        let role = get_member_role(&pool, ws.id, user_b).await.unwrap();
    ...
    async fn test_add_member_idempotent_no_override() {
        ...
        let ws = create(&pool, "Test WS", &slug, "public", user_a).await.unwrap();
        add_member(&pool, ws.id, user_b, "viewer", user_a).await.unwrap();
        add_member(&pool, ws.id, user_b, "editor", user_a).await.unwrap();
        let role = get_member_role(&pool, ws.id, user_b).await.unwrap();
        assert_eq!(role.as_deref(), Some("viewer"), "ON CONFLICT DO NOTHING preserves original role");
    ...
    async fn test_change_member_role_success() {
        ...
        let new_role = change_member_role(&pool, ws.id, user_b, "editor").await.unwrap();
        assert_eq!(new_role.as_deref(), Some("editor"));
        let role = get_member_role(&pool, ws.id, user_b).await.unwrap();
        assert_eq!(role.as_deref(), Some("editor"));

        let new_role = change_member_role(&pool, ws.id, user_b, "viewer").await.unwrap();
    ...
    async fn test_change_member_role_allows_owner_change() {
        ...
        let ws = create(&pool, "Test WS", &slug, "public", user_a).await.unwrap();
        let result = change_member_role(&pool, ws.id, user_a, "editor").await.unwrap();
        assert!(result.is_some(), "DB allows owner role change; guard is at app layer");
        let role = get_member_role(&pool, ws.id, user_a).await.unwrap();
        assert_eq!(role.as_deref(), Some("editor"));
    ...
    async fn test_create_invitation_with_role() {
        ...
        let inv = create_invitation(&pool, ws.id, "newuser@test.com", "viewer", user_a, user_b).await.unwrap();
        assert_eq!(inv.email, "newuser@test.com");
        assert_eq!(inv.role, "viewer");
        assert_eq!(inv.status, "pending");
        assert_eq!(inv.invited_by, user_a);

        let inv2 = create_invitation(&pool, ws.id, "newuser2@test.com", "editor", user_a, user_b).await.unwrap();
    ...
    async fn test_accept_invitation_changes_status() {
        ...
        let ws = create(&pool, "Test WS", &slug, "public", user_a).await.unwrap();
        let inv = create_invitation(&pool, ws.id, "newuser@test.com", "editor", user_a, user_b).await.unwrap();
    ...
    async fn test_find_invitation_by_id() {
        ...
        let ws = create(&pool, "Test WS", &slug, "public", user_a).await.unwrap();
        let inv = create_invitation(&pool, ws.id, "findme@test.com", "editor", user_a, user_b).await.unwrap();
    ...
    async fn test_list_members_includes_all_roles() {
        ...
        let ws = create(&pool, "Test WS", &slug, "public", user_a).await.unwrap();
        add_member(&pool, ws.id, user_b, "editor", user_a).await.unwrap();
        add_member(&pool, ws.id, user_c, "viewer", user_a).await.unwrap();
    ...
    async fn test_find_pending_invitations_for_user() {
        ...
        let ws = create(&pool, "Test WS", &slug, "public", user_a).await.unwrap();
        create_invitation(&pool, ws.id, "bob@test.com", "editor", user_a, user_b).await.unwrap();
    ...
    async fn test_add_member_direct_joined_via_direct() {
        ...
        add_member_direct(&pool, ws.id, user_b, "editor", user_a).await.unwrap();
        let row: (String,) = sqlx::query_as(
            "SELECT joined_via FROM workspace_members WHERE workspace_id = $1 AND user_id = $2"
        )
        .bind(ws.id).bind(user_b)
    ...
    async fn test_add_member_public_join_joined_via_public_join() {
        ...
        add_member_public_join(&pool, ws.id, user_b, "viewer", user_a).await.unwrap();
        let row: (String,) = sqlx::query_as(
            "SELECT joined_via FROM workspace_members WHERE workspace_id = $1 AND user_id = $2"
        )
        .bind(ws.id).bind(user_b)
    ...
    async fn test_resolve_user_identifier_by_uuid() {
        ...
        let resolved = resolve_user_identifier(&pool, &user_a.to_string()).await.unwrap();
    ...
    async fn test_resolve_user_identifier_by_email() {
        ...
        let resolved = resolve_user_identifier(&pool, "alice@test.com").await.unwrap();
        assert_eq!(resolved, Some(user_a));
    }
    ...
    async fn test_resolve_user_identifier_not_found() {
        ...
        let resolved = resolve_user_identifier(&pool, "nonexistent@test.com").await.unwrap();
    ...
    async fn test_resend_invitation_updates_count_and_expiry() {
        ...
        let inv = create_invitation(&pool, ws.id, "bob@test.com", "editor", user_a, user_b).await.unwrap();
    ...
    async fn test_revoke_invitation_sets_expired() {
        ...
        let inv = create_invitation(&pool, ws.id, "bob@test.com", "viewer", user_a, user_b).await.unwrap();
    ...
    async fn test_expire_stale_invitations() {
        ...
        assert!(count >= 1, "should have expired at least one stale invitation");
    ...
    async fn test_find_invitations_by_workspace() {
        ...
        create_invitation(&pool, ws.id, "a@test.com", "viewer", user_a, user_b).await.unwrap();
        create_invitation(&pool, ws.id, "b@test.com", "editor", user_a, user_b).await.unwrap();
    ...
    async fn test_invalid_role_rejected_by_check_constraint() {
        ...
        .bind(ws.id).bind(user_b).bind(user_a)
        .execute(&pool).await;
        assert!(result.is_err(), "role 'writer' should be rejected by CHECK constraint");
    ...
    async fn test_all_new_roles_accepted_by_check_constraint() {
        ...
        for role in ["owner", "manager", "editor", "viewer"] {
            ...
            .execute(&pool).await;
            assert!(result.is_ok(), "role '{role}' should be accepted: {result:?}");
        }
