...
mod tests {
    ...
    async fn add_member_to_ws(pool: &PgPool, ws_id: Uuid, user_id: Uuid, role: &str) {
        ...
        .bind(ws_id).bind(user_id).bind(role).bind(user_id)
        .execute(pool).await;
    }
    ...
    async fn test_transfer_lifecycle_pending_to_accepted() {
        ...
        // Create transfer
        let t = create_transfer(&pool, ws_id, owner, target, "manager").await.unwrap();
        ...
        .bind(ws_id).bind(target)
        .fetch_one(&pool).await.unwrap();
        assert_eq!(new_owner_role.0, "owner", "target should be promoted");

        // Cleanup
        let _ = sqlx::query("DELETE FROM workspaces WHERE id = $1").bind(ws_id).execute(&pool).await;
    }
    ...
    async fn test_transfer_reject() {
        ...
        let t = create_transfer(&pool, ws_id, owner, target, "viewer").await.unwrap();
    ...
    async fn test_find_pending_for_user_only_returns_pending() {
        ...
        let t1 = create_transfer(&pool, ws_id, owner, target, "viewer").await.unwrap();
        ...
        assert!(!pending_after.iter().any(|t| t.id == t1.id), "accepted transfers should not appear");
